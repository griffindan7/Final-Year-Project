...
const BEACON_ID = "B8:27:EB:F0:53:EE"
...
const BeaconScreen = ( { route, navigation } ) => {
  ...
  const handleDiscoverPeripheral = (peripheral: Peripheral) => {
    ...
    if (peripheral.id = "B8:27:EB:F0:53:EE") {
      peripheral.name = 'Beacon';
      //txPowerLevel = 7;
      //n = 1;
      //x = 10;
      //y = ((txPowerLevel - peripheral.rssi) / (10 * n))
      //distance = Math.pow(x, y);
      distance = getRandomFloat(1, 3, 2);
    }
    addOrUpdatePeripheral("B8:27:EB:F0:53:EE", peripheral);
  };
  ...
  const renderItem = ({item}: {item: Peripheral}) => {
    ...
    return (
      <TouchableHighlight
        ...
        onPress={() => navigation.navigate('Mapping', { beacon: "Beacon 1", distance: distance, })}>
        <View style={[styles.row, {backgroundColor}]}>
          ...
          <Text style={styles.peripheralId}>ID: {item.id}</Text>
          <Text style={styles.distance}>Distance: {distance} metres</Text>
        </View>
      </TouchableHighlight>
    );
  };
